...
class YoyakuPlayerUltraFin {
    ...
    initWaveSurfer() {
        ...
        try {
            ...
            this.wavesurfer.on("ready", () => {
                // CRITICAL: Check if ready already handled for this track load
                if (this.waveformReady) {
                    console.log("WaveSurfer ready event already handled, skipping duplicate");
                // 🎯 BENJAMIN BUSINESS FIX: Force play after duplicate ready
                if (this.autoPlayAfterLoad) {
                    console.log('ud83cudfc6 BUSINESS FIX: Force play after duplicate ready');
    ...
    initHTML5Audio() {
        ...
        this.audio.addEventListener('ended', () => {
            this.nextTrack(); // Fixed: Restore auto-advance
            console.log('🛑 Track ended - STOP (no auto-advance)');
            this.pause();
            this.isPlaying = false;
            this.updatePlayButton();
        });
        ...
        setTimeout(() => {
            ...
            const waveform = document.querySelector('.yoyaku-ultrafin-waveform');
            const fakeWaveform = document.querySelector('.fake-waveform');
            
            // Add click handler to multiple elements for better coverage
            [waveformContainer, waveform, fakeWaveform].forEach(element => {
                if (element) {
                    element.addEventListener('click', (e) => {
                        if (this.audio && this.audio.duration) {
                            ...
                            console.log(`Waveform clicked! Seeking to ${(percentage * 100).toFixed(1)}%`);
                            this.showStatus(`Seeking to ${this.formatTime(percentage * this.audio.duration)}`);
                        }
                    });
                }
            });
            
            console.log('Enhanced waveform click handlers added');
        }, 500);
        
        // Update waveform colors based on progress (no time display)
        this.audio.addEventListener('timeupdate', () => {
            if (this.audio.duration) {
                const percentage = (this.audio.currentTime / this.audio.duration) * 100;
                
                // Update waveform bar colors
    ...
    bindEvents() {
        ...
        // Touch events pour mobile/tablet - Benjamin fix
        document.querySelector(".playlist-toggle").addEventListener("touchstart", (e) => {
            e.preventDefault();
            document.querySelector(".playlist-container").classList.toggle("open");
        });
    ...
    loadTrack(index) {
        this.waveformReady = false;  // Reset for new track
        ...
        if (this.wavesurfer) {
            ...
            // CRITICAL: Stop and empty the current waveform before loading new track
            this.wavesurfer.stop();  // Stop playback and reset to beginning
            this.wavesurfer.empty(); // Clear the waveform
    ...
    play() {
        ...
        // 🎵 BENJAMIN AudioContext FIX: Resume suspended context
        ...
        if (this.wavesurfer) {
            // 🎵 FORCE AudioContext resume avant play
            if (this.wavesurfer.backend && this.wavesurfer.backend.ac && this.wavesurfer.backend.ac.state === 'suspended') {
    ...
    nextTrack() {
        ...
        if (this.currentTrackIndex < this.tracks.length - 1) {
            ...
            // BENJAMIN FIX: Auto-play seulement si c'était en cours de lecture
            this.autoPlayAfterLoad = true; // BENJAMIN: Next TOUJOURS auto-play
        } else {
